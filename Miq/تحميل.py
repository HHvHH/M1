...
async def song(client, message: Message):
    ...
    if not urlissed:
        ...
        return
    pablo = await client.send_message(message.chat.id, f"**🔎 جاري تحميل : @deoou ** `{urlissed}`")
...
async def vsong(client, message: Message):
    ...
    pablo = await client.send_message(message.chat.id, f"**🔎 يبحث عن : @deoou** `{urlissed}`")
    if not urlissed:
    ...
    try:
        ...
        await event.edit(event, f"**التحميل فشل : @deoou** \n `{str(e)}`")
        return
    c_time = time.time()
